...
import { generateWebSiteSchema } from '@/lib/seo-generators';
...
export const metadata: Metadata = constructMetadata({
  ...
  description:
    'Build production-grade AI apps with a progressive web app core, battle‑tested SEO, and env‑driven branding. Next.js 15 + React 19 starter.',
  pathname: '/',
  contentType: 'website',
});
...
function LoadingIllustrationSSR() {
  ...
  return (
    <div className="relative h-full w-full">
      <Image
        src={src}
        alt="AIFA - Enterprise AI starter kit hero illustration"
        width={800}
        height={600}
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-16 px-6 py-12 sm:py-16">
        ...
        <section className="grid items-center gap-8 sm:grid-cols-2" aria-labelledby="hero-title">
          <div className="space-y-5">
            ...
            >
              {appConfig.name} — Enterprise AI Next.js Starter
            </h1>

            {/* Hero description */}
            <p className="text-base text-muted-foreground sm:text-lg">
              Build production-grade AI apps with a progressive web app core, battle‑tested SEO,
              and env‑driven branding.
            </p>
        ...
        <section aria-labelledby="explore-title" className="space-y-6">
          ...
          <nav className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <Link
              href="/blog"
              className="rounded-lg border border-border bg-card p-4 hover:bg-muted focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
            >
        ...
        <section aria-labelledby="faq-title" className="space-y-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Hint
              title="How do I change the site name and description?"
              lines={[
                'Set NEXT_PUBLIC_APP_NAME and NEXT_PUBLIC_APP_DESCRIPTION in .env.local',
                'Brand is auto-injected into metadata, manifest, and UI.',
              ]}
            />
            <Hint
              title="How to enable Search Console verifications?"
              lines={[
                'Add NEXT_PUBLIC_GOOGLE_VERIFICATION and NEXT_PUBLIC_YANDEX_VERIFICATION',
                'They are added to metadata only when non-empty.',
              ]}
            />
            <Hint
              title="Can I restrict AI crawlers?"
              lines={[
                'Edit app/robots.ts and replace allow → disallow for GPTBot, PerplexityBot, etc.',
                'Default configuration allows AI bots for better AI search visibility.',
              ]}
            />
            <Hint
              title="How to add dynamic routes to the sitemap?"
              lines={[
                'Uncomment examples in app/sitemap.ts and fetch from your database/APIs.',
                'Remember the 50k URLs per sitemap limit.',
              ]}
            />
          </div>
        </section>
